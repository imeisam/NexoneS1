    ...
    window.addEventListener('message', (event) => {
		...
		switch(data.action) {
           ...
           case "open":
                            ...
                            " data-bone="${v.bone}" data-w="${w}"> 
                                ...
                                <div class="stap">
                                    <svg viewBox="0 0 184 68" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <circle cx="3" cy="65" r="3" fill="#5CDBAD"/>
                                        <circle cx="181" cy="3" r="3" fill="#5CDBAD"/>
                                    </svg>
                                </div>
                            </div>
                        `);
                    ...
                    $.each(configg.BoneLabelText, function (q, w) { 
                        if(w.bid == v.bid) {
                            ...
                            if(binfo != null) {
                                ...
                                $("." + binfo.name.toLowerCase() + "-status").css("width" , health+ "%");
                            }
                        }
                    });
            ...
            $(".ic-liste").html("");
            $.post("https://qb-realisticdisease/checkItems", {},  function (data) {
           ...
           case "checked":
                ...
                $.each(CurrentInjure, function (i, v) { 
                    ...
                    $("." + binfo.unity + "_doc").show("fade");
                });

           break;
           case "theme":
                if(data.reverse == true) {
                    $(".hud").css("transform", "scaleX(-1)");
                    $(".human_doc").css("transform", "scaleX(-1)");
                }
                if(data.theme == "red") {
                    $(".style").html(`
                        .ic-img, .dot {
                        ...
                        .status-prog {
                            ...
                            text-shadow: 0px 0px 20px #db5c69;
                        }
                        .status-c {
                            background: #dc5d5d63;
                        }
                        .bone-item-heal {
                            background: #DB5C69;
                            box-shadow: 0px 0px 7px 0px #db5c69a3;
                        }
                        .com-ic-listx button {
                            background: #DB5C69;
                            box-shadow: 0px 0px 7px 0px #db5c69a3;
...
$(document).on("click", ".com-ic-listx button" , function() {
    $.post("https://qb-realisticdisease/rev",  JSON.stringify(cacheid));
    Close();
});  
...
function handle() {
    ...
    $(".bone-item").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("https://qb-realisticdisease/checkTreatment", JSON.stringify({ bone: bone, area: w, item: item, cacheid: cacheid }),  function (data) {
            ...
            $.post("https://qb-realisticdisease/close");
            clearInterval(interval);
            interval = null;
        }
    });
