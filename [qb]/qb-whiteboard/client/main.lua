    ...
    for k,v in pairs(Config.Locations) do
        ...
        Combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dui[k] then
                    dui[k] = getDui(v.currentImage)
                    AddReplaceTexture(v.origTxd, v.origTxn, dui[k].dictionary, dui[k].texture)
                else
                    changeDuiUrl(dui[k].id, v.currentImage)
                end
                Config.Locations[k].idzone = k
...
RegisterNetEvent("qb-whiteboard:changewhiteboardcli", function(pUrl, pRoom)
    if Config.Locations[pRoom] ~= nil then
        ...
        if Config.Locations[pRoom].inZone  then
            if Config.Locations[pRoom].idzone and dui[Config.Locations[pRoom].idzone] then
            changeDuiUrl(dui[Config.Locations[pRoom].idzone].id, Config.Locations[pRoom].currentImage)
            end
        end
    end
end)
