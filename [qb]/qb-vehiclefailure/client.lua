...
RegisterNetEvent('qb-vehiclefailure:client:RepairVehicleFull', function(slot)
	local veh = QBCore.Functions.GetClosestVehicle()
	if veh ~= nil and veh ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
		...
		if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
			...
			if #(pos - drawpos) < 2.0 and not IsPedInAnyVehicle(ped) then
				local success = exports["five-repairkit"]:Minigame(1)
				if not  success then
					return
				end
				...
				QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(15000, 25000), false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					TriggerServerEvent("inventory:server:breakItem", "advancedrepairkit", 50, slot)
					if (IsBackEngine(GetEntityModel(veh))) then
...
RegisterNetEvent('iens:repaira', function()
	local ped = PlayerPedId()
	local vehT = GetVehiclePedIsIn(ped, false)
	if vehT and vehT ~= 0 then
		vehicle = vehT
