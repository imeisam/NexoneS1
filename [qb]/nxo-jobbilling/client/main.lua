...
RegisterNetEvent('nxo:client:openbill', function ()
    local xPlayer =  QBCore.Functions.GetPlayerData()
    SetNuiFocus(true, true)
    SendNUIMessage({
        action = 'openMenu',
...
RegisterNetEvent('jerzys_billing:client:sendBilling', function (ammount, title, text, name, cId)
    ...
    local success = exports['qb-phone']:PhoneNotification("Give Bill", "You have a $"..ammount..' '..title, 'fas fa-phone', '#b3e0f2', "NONE", 'fas fa-check-circle', 'fas fa-times-circle')
    if success then
