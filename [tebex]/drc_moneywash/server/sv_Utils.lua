...
local webhook =
"https://discord.com/api/webhooks/1312811069047312426/MjgRDKEDM_A_n816pTR8MzJMhqpTpVXVjO3Yhodkk5DgK-bIo2FKg14Yjb5iWenM5fkpO"
...
function GetMoney(count, source)
    ...
    elseif Config.Framework == "qbcore" then
        local xPlayer = QBCore.Functions.GetPlayer(source)
        if xPlayer.Functions.GetCash() >= count then
            return true
        else
            return false
        end
...
function RemoveItem(name, count, source)
    ...
    elseif Config.Framework == "qbcore" then
        ...
        TriggerEvent('mh-cashasitem:server:updateCash', src, name, count, "remove", true)
    elseif Config.Framework == "standalone" then
        -- ADD YOUR FRAMEWORK
    end
end
            ...
            if access and nickname_data == nil then
                ...
                if not charAlreadyExists then
                   ...
                   local houseName = house
                    ...
                    if fetchResult[1] and fetchResult[1].access then
                        local accessData = fetchResult[1].access
                        local accessList = json.decode(accessData)

                        for _, accessEntry in pairs(accessList) do
                            if accessEntry.player and accessEntry.player == char then
                                table.insert(AccessHouses, houseName)
                                break
                            end
                        end
                    end

                    if #AccessHouses > 0 then
                        TriggerClientEvent('drc_moneywash:createblip', src, AccessHouses)
                        TriggerClientEvent('drc_moneywash:setaccess', src, AccessHouses)
                    end
