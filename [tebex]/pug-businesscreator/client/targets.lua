...
RegisterNetEvent("Pug:Client:DoBusinessCraftLogic", function(Data)
	...
	for k, v in pairs(AllCraftableData) do
		...
		if Config.Menu == "ox_lib" then
			local Image = 'https://cfx-nui-'..Config.InventoryType..'/html/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
			local Icon = 'https://cfx-nui-'..Config.InventoryType..'/html/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
			if Config.InventoryType == "ox_inventory" then
				Image = 'https://cfx-nui-ox_inventory/web/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
				Icon = 'https://cfx-nui-ox_inventory/web/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
				if not string.find(Image, ".png") then
					Image = Image..".png"
					Icon = Icon..".png"
				end
			elseif Config.InventoryType == "codem-inventory" then
				Image = 'https://cfx-nui-codem-inventory/html/itemimages/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
				Icon = 'https://cfx-nui-codem-inventory/html/itemimages/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
			elseif Config.InventoryType == "ak47_qb_inventory" then
				Image = 'https://cfx-nui-'..Config.InventoryType..'/web/build/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
				Icon = 'https://cfx-nui-'..Config.InventoryType..'/web/build/images/'..GetItemsInformation(AllCraftableData[k].MainIngredientItem, true)
			end
...
RegisterNetEvent('Pug:client:AddItemsToShop', function(Data)
    ...
    local function hasEnoughItem(selectedItem, amount)
        if Config.InventoryType == "ox_inventory" then
            local count = exports.ox_inventory:GetItemCount(selectedItem)
            return count and count >= amount
        elseif Config.InventoryType == "qs-inventory" then
            local count = exports['qs-inventory']:Search(selectedItem)
			return count and count >= amount
        elseif Config.InventoryType == "codem-inventory" then
    ...
    if Config.Input == "ox_lib" then
        ...
        if input then
            ...
            if selectedItem and amount and cost then
                if hasEnoughItem(selectedItem, amount) then
                    if Stored then
