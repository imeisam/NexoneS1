...
local function formatCharacterName(playerSource)
    local name = Framework.GetPlayerFromId(playerSource)?.getName()
...
local function getGroupMembersCount(groupId)
    ...
    local groupcount = 0
    if Groups[groupId]["members"]["leader"] then
        groupcount += 1
    end

    if Groups[groupId]["members"]["helpers"] ~= nil then
        for _, _ in pairs(Groups[groupId]["members"]["helpers"]) do
            groupcount += 1
        end
    end
...
local function changeGroupLeader(groupId)
    ...
    if leader ~= 0 then
        ...
        TriggerClientEvent('yseries:client:send-notification', leader, notification)
    end
...
local function groupExists(groupId)
    ...
    for k, v in pairs(Groups) do
        if Groups[k] ~= nil then
            if v.status == "WAITING" then
                ...
                activeGroups[#activeGroups + 1] = groupData
            end
        end
    end
    ...
    if not Players[src] then
        if Groups[groupId] then
            if #Groups[groupId]["members"] < Config.GroupMembersLimit then
                ...
                Requests[groupId][#Requests[groupId] + 1] = src
...
lib.callback.register("yseries:server:groups:get-requests", function(_, groupId)
    ...
    if Requests[groupId] then
        for _, v in pairs(Requests[groupId]) do
            requests[#requests + 1] = { name = formatCharacterName(v), id = v }
        end
...
exportHandler('yseries', "GetGroupLeader", getGroupLeader)

exports("IsGroupLeader", isGroupLeader)
exportHandler('yseries', "IsGroupLeader", isGroupLeader)

exports('GetGroupMembers', getGroupMembers)
exportHandler('yseries', 'GetGroupMembers', getGroupMembers)

exports("FindGroupByMember", findGroupByMember)
exportHandler('yseries', "FindGroupByMember", findGroupByMember)

exports('GetJobStatus', getJobStatus)
exportHandler('yseries', 'GetJobStatus', getJobStatus)

exports('SetJobStatus', setJobStatus)
exportHandler('yseries', 'SetJobStatus', setJobStatus)

exports('GetGroupMembersCount', getGroupMembersCount)
exportHandler('yseries', 'GetGroupMembersCount', getGroupMembersCount)

exports('CreateBlipForGroup', createBlipForGroup)
exportHandler('yseries', 'CreateBlipForGroup', createBlipForGroup)

exports('RemoveBlipForGroup', removeBlipForGroup)
exportHandler('yseries', 'RemoveBlipForGroup', removeBlipForGroup)

exports("SendGroupEvent", sendGroupEvent)
exportHandler('yseries', "SendGroupEvent", sendGroupEvent)

exports("SetGroupData", setGroupData)
exportHandler('yseries', "SetGroupData", setGroupData)

exports("GetGroupData", getGroupData)
exportHandler('yseries', "GetGroupData", getGroupData)

exports("DestroyGroupData", destroyGroupData)
exportHandler('yseries', destroyGroupData)

exports('NotifyGroup', notifyGroup)
exportHandler('yseries', 'NotifyGroup', notifyGroup)

exports('GroupExists', groupExists)
exportHandler('yseries', 'GroupExists', groupExists)
