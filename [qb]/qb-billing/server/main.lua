...
RegisterNetEvent('qb-billing:payBill:server', function(id, amount, type, sender)
    ...
    if myMoney >= amount then
        ...
        local senderMoney =amount
        local playerbil = GetPlayer(src)
        if Config.InvoiceJobs[type] then
            local senderMoney = amount * (Config.InvoiceJobs[type].commision / 100)
            exports["snipe-banking"]:AddMoneyToAccount(type, amount-senderMoney,"Bill From "..playerbil.PlayerData.charinfo.firstname.." "..playerbil.PlayerData.charinfo.lastname)
        end
        ...
        local player = GetPlayerByCid(sender)
            if player then
                ...
                if CoreName == "qb-core" or CoreName == "qbx_core" then
                    AddMoney(player.PlayerData.source, "bank", math.floor(senderMoney), "bill-commision From "..playerbil.PlayerData.charinfo.firstname.." "..playerbil.PlayerData.charinfo.lastname)
                elseif CoreName == "es_extended" then
                    AddMoney(player.source, "bank", math.floor(senderMoney ), "bill-commision")
                end
            else
                AddMoneyOffline(sender, math.floor(senderMoney))
            end
