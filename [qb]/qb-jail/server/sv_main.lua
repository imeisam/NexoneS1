...
local unJailPlayer = function(src)
    ...
    local myInventory = Player.PlayerData.items
    Player.Functions.ClearInventory()
    ...
    for _, v in pairs(myInventory) do
        Player.Functions.AddItem(v.name, v.amount, nil, v.info, v.weight)
    end
...
SentenceLoop = function()
    for k, v in pairs(jailedPlayers) do
        if v and jailedPlayers[k]>0 then
            jailedPlayers[k] -= 1
            if jailedPlayers[k] <= 0 then
                jailedPlayers[k] =-1
                TriggerClientEvent('QBCore:Notify', k, 'Your sentence is up. You have been released.', 'primary', 4000)
            else
                TriggerClientEvent('QBCore:Notify', k, 'You Have '..jailedPlayers[k]..' Minutes Left In Your Jail Time', 'primary', 4000)
            end
...
AddEventHandler('onResourceStop', function(resource)
    ...
    for i=1, #Config.gardenprop do
        if DoesEntityExist(Config.gardenprop[i].handle) then
            DeleteEntity(Config.gardenprop[i].handle)
        end
    end
...
RegisterNetEvent('qb-jail:server:RequestSentenceReduction', function(sentGroup)
    ...
    if sentGroup == getPlayerJobGroup(src) then
        reduceJailSentenceAttempt(source)
    end
end)
...
QBCore.Commands.Add('confiscate', 'Confiscate Player Items', {{name = 'id', help = 'Player ID'}}, true, function(source, args)
    ...
    if Player and OtherPlayer and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then -- if Player and OtherPlayer and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then
        local ped = GetPlayerPed(src)
