...
AddEventHandler('qb-pursuitmode:client:changeVehicleMode', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if DoesEntityExist(vehicle) and isValidVehicle(vehicle) and isAuthorizedToSwitchMode() then
            local vehicleModel = getModelFromHash(GetEntityModel(vehicle))
           if not Config.VehiclesConfig[vehicleModel] then return end
            ...
            if currentVehicleMode=="A" then
                TriggerEvent('nxo-hud:client:changevalue', 10)
            elseif currentVehicleMode=="A+" then
                    TriggerEvent('nxo-hud:client:changevalue', 30)
            elseif currentVehicleMode=="S" then
                    TriggerEvent('nxo-hud:client:changevalue', 60)
            elseif currentVehicleMode=="S+" then
                    TriggerEvent('nxo-hud:client:changevalue', 100)
            end
            local Player = QBCore.Functions.GetPlayerData()
            TriggerServerEvent("qb-log:server:CreateLog", "pursuitmode", "Pursuit Mode: "..currentVehicleMode , "green", "**".. Player.charinfo.firstname..' '..Player.charinfo.lastname .. "** (citizenid: *"..Player.citizenid.."*) \n Vehicle Model : **".. QBCore.Shared.Vehicles[vehicleModel].name.."**\n Plate: **"..GetVehicleNumberPlateText(vehicle).."**" )
...
AddEventHandler("baseevents:leftVehicle",function(PreviousVehicle, PreviousSeat, name, class, model)
	if vehpersuit == nil then return end
	if PreviousVehicle ==  vehpersuit then
        TriggerEvent('nxo-hud:client:changevalue', 10)
        currentVehicleMode="A"
        gear=1
        updateHandling(PreviousVehicle)
        applyVehicleMods(PreviousVehicle)
        vehpersuit=nil
	end
