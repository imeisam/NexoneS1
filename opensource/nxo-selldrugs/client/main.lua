...
Trade_Animation = {{dict = "mp_common", anim = "givetake1_a", duration = 2000, swap = 1000, Offset = {x = 0.0, y = 0.9, z = 0.0}}, {dict = "mp_ped_interaction", anim = "handshake_guy_a", duration = 3000, swap = 1500, Offset = {x = 0.1, y = 1.05, z = 0.0}}}
...
RegisterNetEvent('SellDrugs:client:Status_Selling_Drugs', function()
    TriggerServerEvent("SellDrugs:Status_Selling_Drugs")
end)
local QBCore = exports['qb-core']:GetCoreObject()
RegisterNetEvent('inventory:client:closedinv', function()
    if cfg.Police.Job.name=='police' or cfg.Police.Job.name=='ambulance' or cfg.Police.Job.name=='justice' or cfg.Police.Job.name=='goverment' then
        exports['qb-radialmenu']:RemoveOption('Selldrug')
    else
        local hasitem=false
        for k, v in pairs(cfg.Drug_List) do
        ...
        if hasitem then
            exports['qb-radialmenu']:AddOption({
                ...
                icon = '#cannabis',
                functiontype = 'client',
                functionName = 'SellDrugs:client:Status_Selling_Drugs',
                shouldClose = true
            }, 'Selldrug')
        else
            exports['qb-radialmenu']:RemoveOption('Selldrug')
