...
RegisterNetEvent('wasabi_crutch:client:pickupwheelchair', function(data)
    QBCore.Functions.Progressbar("search_register", "Pickup Wheelchair...", 1000, false, true, {
        ...
        TriggerServerEvent('wasabi_crutch:additem', Config.usableWheelchairItem.item)
    end, function()
        
    end)
   
end)
RegisterNetEvent('wasabi_crutch:client:spawnwheelchair', function()
    local coords = GetEntityCoords(cache.ped)
    local hash = `iak_wheelchair`
    lib.requestModel('iak_wheelchair', 100)
    local heading = GetEntityHeading(cache.ped)
    chairVeh = CreateVehicle(hash, coords.x+0.5, coords.y+0.5, coords.z-1, heading, 1, 0)
    ...
    TriggerServerEvent('wasabi_crutch:removeItem', Config.usableWheelchairItem.item)
     ...
     exports['qb-target']:AddTargetEntity(chairVeh, {
        options = {
            {
                type = "client",
                event = "wasabi_crutch:client:pickupwheelchair",
                icon = "fas fa-exchange-alt",
                label = "Store Wheelchair",
                entity=chairVeh,
            }
        },
        distance = 2.5
    ...
    CreateThread(function()
        ...
        while true do
            ...
            if not DoesEntityExist(chairVeh) and not InVehicle then
                ...
                local hash = `iak_wheelchair`
                lib.requestModel('iak_wheelchair', 100)
                chairVeh = CreateVehicle(hash, coords.x, coords.y, coords.z-1, heading, 1, 0)
