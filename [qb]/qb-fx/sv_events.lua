...
AddEventHandler("fx:ThermiteChargeEnt", function(pNetId)
    ...
    local position = {
        coords = { { x = pCoords.x + 0.5, y = pCoords.y + 0.5, z = pCoords.z } },
...
RegisterServerEvent("qb-fx:setSteamId")
AddEventHandler("qb-fx:setSteamId", function()
    local src = source
    ...
    TriggerClientEvent("qb-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("qb-fx:smoke:grenade")
AddEventHandler("qb-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("qb-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
    ...
    if pType == "ruby" then
        ptDict, ptName = "core", "fire_wrecked_plane_cockpit"
    end
