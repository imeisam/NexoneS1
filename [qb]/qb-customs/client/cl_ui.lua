...
function InitiateMenus(isMotorcycle, vehicleHealth, categories, welcomeLabel)
    ...
    if vehicleHealth < 1000.0 and categories.repair then
        ...
        populateMenu("repairMenu", -1, "Repair", "Free")
    ...
    for _, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15 or v.id == 16 then --Performance Upgrades
                ...
                for _, n in pairs(validMods) do
                    ...
                    if maxVehiclePerformanceUpgrades == 0 then
                        populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "Free")
            ...
            elseif v.id == 18 then
                ...
                populateMenu(v.category:gsub("%s+", "") .. "Menu", -1, "Disable", "Free")
                populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Enable", "Free")
    ...
    for _, v in ipairs(vehicleResprayOptions) do
        ...
        for _, n in ipairs(v.colours) do
            populateMenu(v.category .. "Menu", n.id, n.name, "Free")
        end
    ...
    for _, v in ipairs(vehicleWheelOptions) do
        if v.id == -1 then
            ...
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Disable", "Free")
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Enable", "Free")
    ...
    for k, v in ipairs(vehicleTyreSmokeOptions) do
        populateMenu("TyreSmokeMenu", k, v.name, "Free")
    ...
    for _, v in ipairs(vehicleWindowTintOptions) do
        populateMenu("WindowTintMenu", v.id, v.name, "Free")
    ...
    if livCount > 0 then
        ...
        for i=0, livCount-1 do
            populateMenu("OldLiveryMenu", i, "Livery", "Free")
    ...
    for i=0, #plateTypes-1 do
        if i ~= 4 or (i == 4 and GetVehicleClass(plyVeh) == 18) then
            populateMenu("PlateIndexMenu", i, plateTypes[i+1], "Free")
    ...
    for _, v in ipairs(vehicleNeonOptions.neonTypes) do
        ...
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 0, "Disabled", "Free")
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Enabled", "Free")
    ...
    for k, _ in ipairs(vehicleNeonOptions.neonColours) do
        populateMenu("NeonColoursMenu", k, vehicleNeonOptions.neonColours[k].name, "Free")
    ...
    populateMenu("HeadlightsMenu", 0, "Disable Xenons", "Free")
    populateMenu("HeadlightsMenu", 1, "Enable Xenons", "Free")
    ...
    for _, v in ipairs(vehicleXenonOptions.xenonColours) do
        populateMenu("XenonColoursMenu", v.id, v.name, "Free")
