...
function LeaveRadioChannel()
    ...
    elseif Config.Voice.System == "salty" then
        ...
        TriggerServerEvent('yseries:server:radio:leave', currentRadioChannel)
    elseif Config.Voice.System == "toko" then
...
local function onPlyerJoined(playerSrc)
    TriggerServerEvent('yseries:server:radio:player-joined', playerSrc)
end

local function onPlayerLeft(playerSrc)
    TriggerServerEvent('yseries:server:radio:player-left', playerSrc)
end

local function updateParticipantsOnLeave(channel, playerSrc)
    local players = lib.callback.await('yseries:server:radio:get-participants', false, channel)
...
RegisterNuiCallback('radio:join', function(channel, cb)
    ...
    local players = lib.callback.await('yseries:server:radio:get-participants', false, rChannel)
    for source in pairs(players) do
...
-- * Exports
local function isRadioOn() return OnRadio end
exports("IsRadioOn", isRadioOn)
exportHandler('yseries', "IsRadioOn", isRadioOn)
...
RegisterNetEvent('yseries:client:radio:player-joined', function()
    SendUIAction('radio:player-joined')
end)

RegisterNetEvent('yseries:client:radio:player-left', function()
    playersInChannel = playersInChannel - 1
