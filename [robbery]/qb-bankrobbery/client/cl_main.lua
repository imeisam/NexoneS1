...
local HackSettings = {
    ['fleeca'] = {
        time = 15, -- time = time in seconds per puzzle
...
local ThermiteSettings = {
    ...
    -- }
      ['fleeca'] = {
        iterations = 2,
        grid  = 6,
        duration  =20000,-- 10sec to fail
        target  = 9, 
        previewDuration =5000 ,
    },
    ['paleto'] = {
        iterations = 1, -- You want to set this way higher!
        grid  = 5,
        duration  =13000,
        target  = 5,
    ...
    ['pacific'] = {
        iterations = 2, -- You want to set this way higher!
        grid  = 6,
        duration  =15000,
        target  = 6,
...
--- This function uses ps-dispatch functions by default to alert the police when doing a bank heist
...
local OnLaptopHackDone = function(success, bank)
    if success then
        ...
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = "Lionh34rt",
            subject = bank,
            message = "I've received your input codes and will now start disabling the security system..<br />Door Will be opened in 5 minutes.",
            button = {}
        })
        if bank == 'Paleto' then
...
LaptopAnimation = function(bank) -- Globally used
    ...
    local minigame=exports["bl_ui"]:PrintLock(3, {
        grid = 4,
        duration = 30000,
        target = 4
    })

    -- local minigame = exports['nxo-MiniGames']:alphabet(HackSettings[Config.Banks[bank].type].amount, 60000)
    if minigame then
        TriggerServerEvent('qb-bankrobbery:server:LaptopDamage', 'laptopg')
        ...
        DeleteObject(laptop)
        FreezeEntityPosition(ped, false)
        TriggerEvent("QBCore:Notify", "Hack Failed", "error")
    end
end
...
StartThermite = function(bank, index) -- Globally used
    if not Config.Banks[bank].thermite[index].hit then
        ...
        if #(pos - Config.Banks[bank].thermite[index].coords.xyz) < 2 then
            ...
            local res=exports["bl_ui"]:MineSweeper(ThermiteSettings[bankType].iterations, {
                grid = ThermiteSettings[bankType].grid, -- grid 4x4
                duration = ThermiteSettings[bankType].duration, -- 10sec to fail
                target = ThermiteSettings[bankType].target, --target you need to remember
                previewDuration =  ThermiteSettings[bankType].previewDuration --preview duration (time for red mines preview to hide)
            })
            if res then
...
RegisterNetEvent('thermite:UseThermite:bank', function()
    local pos = GetEntityCoords(PlayerPedId())
    if closestBank then
