...
function GetPlayerIdentity(source, cb)
    MySQL.Async.fetchAll("SELECT firstname, lastname, dateofbirth, sex, height FROM users WHERE identifier=?", {GetIdentifier(source)}, function(result)
        cb(result[1])
    end)
end

function DocumentItemHandler(source, item, data)
    TriggerClientEvent("nc_documents:displayDocument", -1, GetEntityCoords(GetPlayerPed(source)), data.metadata)
end