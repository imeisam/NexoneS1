...
RegisterNetEvent('wasabi_bridge:onPlayerDeath')
...
RegisterNetEvent('wasabi_crutch:giveCrutch')
AddEventHandler('wasabi_crutch:giveCrutch', function(time)
	StartCrutchLoop(time)
end)

RegisterNetEvent('wasabi_crutch:giveChair')
AddEventHandler('wasabi_crutch:giveChair', function(time)
	StartChairLoop(time)
end)

RegisterNetEvent('wasabi_crutch:breakLoop')
AddEventHandler('wasabi_crutch:breakLoop', function()
	if not LoopStarted then return end
	BreakLoop = true
end)

RegisterNetEvent('wasabi_crutch:givePlayerCrutch')
AddEventHandler('wasabi_crutch:givePlayerCrutch', function(item)
	if Config.jobRequirement.enabled then
		local found
		for i=1, #Config.jobRequirement.jobs do
			if HasGroup(Config.jobRequirement.jobs[i]) then
				found = true
				break
			end
		end
		if not found then
			TriggerEvent('wasabi_crutch:notify', Strings.not_authorized, Strings.not_authorized_desc, 'error')
		else
			OpenCrutchMenu(item)
		end
	else
...
RegisterNetEvent('wasabi_crutch:givePlayerChair')
AddEventHandler('wasabi_crutch:givePlayerChair', function(item)
	if Config.jobRequirement.enabled then
		local found
		for i=1, #Config.jobRequirement.jobs do
			if HasGroup(Config.jobRequirement.jobs[i]) then
