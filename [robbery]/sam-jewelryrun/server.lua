...
QBCore.Functions.CreateCallback('qb-storerobbery:server:gethighriskcooldown', function(source, cb)
    src=source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid=Player.PlayerData.citizenid
    if not playercooldown.highrisk[cid] then cb(true) return end
    if not playercooldown.highrisk[cid].try<=config.highrisk.maxrob-1 then cb(true) return end
     cb(false)
end)
...
RegisterNetEvent('qb-storerobbery:server:sethotrunstatus', function()
    ...
    local cid=Player.PlayerData.citizenid
        ...
        TriggerEvent('qb-cooldown:server:startglobaltimeout',config.hotrun.cooldown)
...
RegisterNetEvent('qb-storerobbery:server:sethighriskstatus', function()
    ...
    local cid=Player.PlayerData.citizenid
        ...
        TriggerEvent('qb-cooldown:server:startglobaltimeout',config.highrisk.hotruncooldown)
