...
skateParkSetup = {
   ...
   -- [2] = {
    ...
    [5] = {
        ...
        coords = vector3(1640.90, 260.87, 44.56),
        heading = 0.0
    },
    -- [6] = {
    --     model = `prop_skate_quartpipe`,
    ...
    --     coords = vector3(1634.35, 2606.28, 44.56),
    --     heading = 180.0
    -- }
}
...
RegisterNetEvent('qb-jail:server:FinishKitchenTask', function(task, success)
    ...
    if kitchenTasks[task].current >= kitchenTasks[task].max then 
        ...
        for k, v in pairs(getGroupMembers('kitchen')) do
            TriggerClientEvent('qb-jail:client:drawtext', v, 'Task','Current Task: ' .. kitchenTasks[currentKitchenTask].label)
        end
    else
        ...
        for k, v in pairs(getGroupMembers('kitchen')) do
            TriggerClientEvent('qb-jail:client:drawtext', v,'Task', kitchenTasks[currentKitchenTask].label .. ' - Progress: ' .. progress .. '%')
        end
    end
...
cleaningSetup = {
    [1] = {
        ...
        coords = vector3(1760.70, 2499.94, 45.20),
        handle = 0,
        active = false
    },
    [2] = {
    ...
    [3] = {
        ...
        coords = vector3(1768.65, 2490.387, 44.56),
        handle = 0,
        active = false
    },
    [4] = {
        model = `ng_proc_litter_plasbot3`,
        coords = vector3(1763.913, 2483.16, 44.84),
        handle = 0,
        active = false
    },
    [5] = {
    ...
    [17] = {
        ...
        coords = vector3(1758.7192382812, 2485.4580078125, 45.80),
        handle = 0,
        active = false
    },
    [18] = {
    ...
    [31] = {
        model = `ng_proc_litter_plasbot2`,
        coords = vector3(1771.93, 2478.40, 47.69),
        handle = 0,
        active = false
    },
    [32] = {
...
RegisterNetEvent('qb-jail:server:FinishCleaningTask', function(netId, success)
    ...
    if cleaningActives <= 0 then
        ...
        for k, v in pairs(getGroupMembers('cleaning')) do
            TriggerClientEvent('qb-jail:client:drawtext', v,'Task', 'Current Task: Cleaning Cells - Progress: ' .. progress .. '%')
        end
    end
end)
...
RegisterNetEvent('qb-jail:server:PrisonScrap', function(index, success)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - Config.scrapYardSetup[index].coords) > 25 then return end
    reduceJailSentenceAttempt(src)
    if success and Config.ScrapRewardChance >= math.random(100) then
        local item = Config.scrapYardSetup[index].rewards[math.random(#Config.scrapYardSetup[index].rewards)]
        if Config.Inventory == 'exports' then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'you are not chance...', 'success', 2500)
    end
end)
