...
function AddContractMission(src,plate,type,netId)
    ...
    startedContracts[plate]['startedTime'] = addMinutes()
    TriggerClientEvent('av_laptop:notification',src,Lang['app_title'], Lang['vin_delivery_time'],'inform')
...
RegisterServerEvent('av_boosting:pay', function(netId, plates)
    ...
    if not ContractExpired(plates) then
        ...
        if startedContracts[plates]['hacker'] and startedContracts[plates]['hacker'] ~= identifier then
            ...
            if not hackerEXP then hackerEXP = 0 end
        ...
        -- Add group EXP:
        local hasGroup = exports['av_groups']:getPlayerGroup(src, "av_boosting")
        if hasGroup and hasGroup['members'] then
            local members = hasGroup['members']
            local hackerIdentifier = startedContracts[plates]['hacker']
            for i=1, #members do
                local member = members[i]
                local memberIdentifier = member['identifier']
                players[memberIdentifier] = players[memberIdentifier] or {}
                players[memberIdentifier]['busy'] = false
                if memberIdentifier ~= identifier and memberIdentifier ~= hackerIdentifier then
                    local memberXP = Config.Classes[class]["EXP"]['group']
                    local finalXP = math.random(memberXP['min'], memberXP['max'])
                    MySQL.update.await('UPDATE av_boosting SET level = (level + ?) WHERE identifier = ?', {finalXP, memberIdentifier})
                    players[memberIdentifier]['busy'] = false
                    TriggerClientEvent('av_boosting:wipe',tonumber(member['playerId']))
                end
            end
            exports['av_groups']:setGroupState("av_boosting", hasGroup.identifier, Lang['waiting'])
        end
    else
        local driver = GetPlayerName(src)
        local hacker = "None"
        local removeEXP = math.random(Config.Classes[class]["FailEXP"]['min'], Config.Classes[class]["FailEXP"]['max'])
        local currentEXP = MySQL.single.await('SELECT * FROM av_boosting WHERE identifier = ?', {identifier})
        if currentEXP and currentEXP.level then
            local totalEXP = tonumber(currentEXP.level - removeEXP)
            if not totalEXP or totalEXP < 0 then
                totalEXP = 0
            end
            MySQL.update.await('UPDATE av_boosting SET level = ? WHERE identifier = ?', {totalEXP,identifier})
        end
        TriggerClientEvent('av_boosting:wipe',src)
        if startedContracts[plates]['hacker'] and startedContracts[plates]['hacker'] ~= identifier then
            hackerSrc = exports['av_laptop']:getSourceByIdentifier(startedContracts[plates]['hacker'])
        ...
        exports['av_laptop']:Discord(webhook,discord)
        TriggerClientEvent('av_laptop:notification',src,Lang['app_title'], Lang['delivery_expired'],'error')
    end
