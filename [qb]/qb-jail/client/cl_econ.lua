...
RegisterNetEvent('qb-jail:client:OpenCraftingMenu', function()
    ...
    for k, v in ipairs(Config.CraftingCost) do
        local text = ''
        for i=1, #v.items do
            print(v.items[i].item)
            print( v.items[i].amount )
...
-- end)
-- RegisterCommand('deblackout', function(source, args)
...
createPrisonPed = function()
    local coords = vector4(1725.95, 2537.58, 44.55, 121.9)
    ...
    while not HasAnimDictLoaded("timetable@ron@ig_3_couch") do Wait(10) end
    TaskPlayAnim(prisonerPed, "timetable@ron@ig_3_couch", 'base', 8.0, 8.0, -1, 2, 0.0, false, false, false)
    Wait(GetAnimDuration("timetable@ron@ig_3_couch", 'base') * 1000)
