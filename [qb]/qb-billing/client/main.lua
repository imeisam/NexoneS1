    ...
    for k, v in pairs(Config.InvoiceJobs) do
        if k == myJob then
            ...
            if next(nearbyPlayers) ~= nil and next(nearbyPlayers) then
                ...
                for _, idk in pairs(nearbyPlayers) do
                    exports['qb-interact']:addGlobalPlayerInteraction({
                        id = "qb-billing-request-players-".. idk, 
                        distance =5.0, 
                        interactDst = 1.3, 
                        options = {
                            {
                                ...
                                action = function(entity, _, _, serverId)
                                    ...
                                    TriggerEvent("qb-billing:openInvoiceMenu:client",serverId)
                                end,
...
RegisterNetEvent('qb-billing:openInvoiceMenu:client', function(pid)
    -- data=data.args
    local translations = {}
    -- print(data.id)
    ...
    TriggerCallback('qb-billing:getTargetName:server', function(name)
        ...
        SendNUIMessage({action = "openInvoiceMenu", text = pid .. " (" .. name .. ")", target = pid, translations = translations})
    end, pid)
    requestActive = false
    exports["qb-textui"]:hideTextUI()
    for _, id in pairs(nearbyPlayers) do
