...
RegisterNetEvent('jerzys_billing:server:sendBilling', function (data)
    ...
    if currentPlayer ~= nil then
        if billedPlayer ~= nil then
            TriggerClientEvent('jerzys_billing:client:sendBilling',  billedPlayer.PlayerData.source, ammount, title, text, name, cId)
            else
...
RegisterNetEvent('jerzys_billing:server:doneBilling',function(data)
    ...
    local currentPlayerFirstName = currentPlayer.PlayerData.charinfo.firstname..' '..currentPlayer.PlayerData.charinfo.lastname
    ...
    if(Config.useCash == true) then
        ...
        if currentPlayer ~= nil then
            if getBank - ammount >= 0 then
                currentPlayer.Functions.RemoveMoney("bank", ammount, "Pay For ".. paidPlayer.PlayerData.job.label)
                if paidPlayer ~= nil then
                    local com=0

                    local tax= (ammount*taxamount)/100
                    ammount=ammount-tax
                    TriggerEvent("qb-tax:client:getandpaytax",ammount,memo,taxamount)
                    if Config.job[paidPlayer.PlayerData.job.name] then
                        com=ammount-(ammount*Config.job[paidPlayer.PlayerData.job.name].com)
                        exports["snipe-banking"]:AddMoneyToAccount(paidPlayer.PlayerData.job.name, com,"Bill For "..currentPlayerFirstName)
                    end

                   

                    paidPlayer.Functions.AddMoney("bank", ammount-com, "Bill For "..currentPlayerFirstName);
                    TriggerClientEvent("QBCore:Notify", paidPlayer.PlayerData.source, "You have recived $"..ammount-com.."$ from: "..currentPlayerFirstName.." bank!", "success")
                end
            else
                TriggerClientEvent("QBCore:Notify", paidPlayer.PlayerData.source, "Player doesnt have money to pay the bill!", "success")
