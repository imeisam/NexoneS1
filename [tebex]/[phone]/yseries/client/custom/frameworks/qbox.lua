CreateThread(function()
    ...
    function HireEmployee(playerId, cb)
        lib.callback.await('qbx_management:server:hireEmployee', false, playerId, 'job')
    ...
    function FireEmployee(playerId, cb)
        lib.callback.await('qbx_management:server:fireEmployee', false, playerId, 'job')
    ...
    function SetGrade(identifier, newGrade, oldGrade, cb)
        ...
        local updateGrade = lib.callback.await('qbx_management:server:updateGrade', false, identifier, oldGrade, newGrade, 'job')
        cb(true)
    end
    ...
    function ToggleDuty()
        ...
        TriggerServerEvent('yseries:server:logs:create', 'companies', 'Companies', 'Duty status change 🪖', 'companies', message, nil, nil, nil, playerIdentifier)
    ...
    function Notify(source, messageData)
        lib.notify(source, messageData)
    end
end)
