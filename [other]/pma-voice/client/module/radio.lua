...
function setRadioChannel(channel)
	...
	-- exports['kevin-hud']:radioenter(true, channel)
end
...
exports('removePlayerFromRadio', function()
	-- exports['kevin-hud']:radioenter(false, 0) 
	setRadioChannel(0)
end)
...
AddEventHandler('onKeyDown', function(key)
	if key == 'capital' then
		...
		if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
		if not LocalPlayer.state.isLoggedIn or isDead() then return end
		
		if not radioPressed and radioEnabled then
			if radioChannel > 0 then
				-- exports["kevin-hud"]:talking(true, true) 
...
AddEventHandler('onKeyUP', function(key)
	if key == 'capital' then
		...
		if radioPressed then
			-- exports["kevin-hud"]:talking(false, false) 
			radioPressed = false
