...
function CameraLoop()
    ...
    TriggerEvent('evidence:client:inCamera', true)
    CreateThread(function()
        ...
        while camera and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) do
            ...
            elseif IsControlJustPressed(1, 176) then
                ...
                exports['screenshot-basic']:requestScreenshotUpload(hook, 'image', function(data)
                    local image = json.decode(data)
                    camera = false
                    if cameraprop then DeleteEntity(cameraprop) end
                    ClearPedTasks(lPed)
                    local coords = GetEntityCoords(lPed)
                    local streetHash, crossingHash = GetStreetNameAtCoord(coords.x, coords.y, coords.z)
                    local streetName = GetStreetNameFromHashKey(streetHash)
                    TriggerServerEvent("qb-camera:savePhoto",image.filename, streetName)
                    SendNUIMessage({action = "SavePic", pic = json.encode(image.filename)})
        ...
        TriggerEvent('evidence:client:inCamera', false)
        camera = false
...
RegisterNetEvent("qb-camera:usePhoto", function(url, location)
    photo = not photo
...
RegisterNetEvent('qb-camera:useCamera', function()
    camera = not camera
...
RegisterNetEvent('qb-camera:getHook', function(key)
    hook = key
end)