...
local tablemodels = {
  ...
  exports['qb-target']:AddTargetModel(tablemodels, {
	options = {
		{
			type    =  'client',
			event   =  'rcore_pool:start',
		...
		},
		{
			...
			event   =  'rcore_pool:start',
			icon    =  'money-bill-wave',
			label   =  'Start Straight Pool',
			name    =  'startSTRAIGHTPOOL',
			actions='STRAIGHTPOOL',
		},
	},
	distance = 2.5, 
})
  ...
  RegisterNetEvent('rcore_pool:start', function(data)
	print( data.actions)
    if data.actions=='8BALL' then

     TriggerEvent('rcore_pool:setupTable', 'BALL_SETUP_8_BALL')
	elseif data.actions=='STRAIGHTPOOL' then 
