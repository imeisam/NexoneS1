...
Citizen.CreateThread(function()
    while true do
        ...
        if scrapblip then
            ...
            if not mert then
                exports['nxo-chopshop']:SetupDigiScanner(scanner, {})
                mert = true
            end
            if dist <= 3 then
                if not copsCalled then
                    ...
                    TriggerServerEvent("nxo-chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                    copsCalled = true
                end
                Citizen.Wait(5000)
                QBCore.Functions.Notify(Config.Locale["FoundVeh"], 'success')
                exports["nxo-oui"]:Show(Config.Locale["title"], Config.Locale["chop3"])
                Citizen.Wait(10000)
...
RegisterNetEvent('orbit-chopshop:jobaccept')
AddEventHandler('orbit-chopshop:jobaccept', function()
    ...
    if not QBCore.Functions.HasItem('weapon_digiscanner') then
		QBCore.Functions.Notify("Not Enough Item For Start", "error")
		return
	end
    ...
    if cooldown == false then
        if LicensePlate ~= nil then
            ...
            randomCoords = math.random(1,5) --EDIT
        ...
        TriggerServerEvent('nxo-chopshop:server:log')
    else
...
function SpawnVehicle(vehicle, x, y, z, w)
    ...
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        ...
        exports["nxo-oui"]:Show(Config.Locale["title"], Config.Locale["chop1"])
        ...
        if Config.Email == true then
            ...
            exports["nxo-oui"]:Show(Config.Locale["title"],
...
function ScrapVehicle()
    ...
    if GetVehicleNumberPlateText(veh) ~= LicensePlate then
        ...
        exports["nxo-oui"]:Show(Config.Locale["title"], Config.Locale["chop4"])
        StartChopping()
        DeleteBlip()
    end
end
...
RegisterNetEvent('orbit-chopshop:client:robberyCall')
AddEventHandler('orbit-chopshop:client:robberyCall', function(type, key, streetLabel, coords)
...
function UpdateBars(dist, plate)
    ...
    if dist < 1.0 then
        ...
        -- end
        if not mert then
            local reshack=exports['nxo-MiniGames']:alphabet(20,10000)
			if not reshack then
            ...
            exports['qb-dispatch']:chopshop(vehchopshop)
            -- exports["orbit-thermite"]:thermiteminigame(6, 3, 2, 20,
            --     function()
                    ...
                    if not copsCalled then
                        ...
                        TriggerServerEvent("orbit-chopshop:server:callCops", "Chopshop", 0, streetLabel, pos)
                        copsCalled = true
                    end
                    Citizen.Wait(5000)
                    QBCore.Functions.Notify(Config.Locale["FoundVeh"], 'success')
