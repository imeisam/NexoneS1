...
CreateThread(function()
    ...
    exports['qb-textui']:create3DTextUI("pa-test", {
    ...
    exports['qb-textui']:delete3DTextUI("pa-test")
    -- For Players
    exports['qb-textui']:create3DTextUIOnPlayers("pa-test", {
        id = targetId,
        ped = targetPed,
        displayDist = 6.0,
        interactDist = 2.0,
    ...
    exports['qb-textui']:delete3DTextUIOnPlayers(targetId)
     -- For Entities, Vehicles, Peds (fixed coordinate)
    exports['qb-textui']:create3DTextUIOnEntity(entity, {
        displayDist = 6.0,
        interactDist = 2.0,
        enableKeyClick = true, -- If true when you near it and click key it will trigger the event that you write inside triggerData
        keyNum = 38,
    ...
    exports['qb-textui']:delete3DTextUI(entity)
end)
```
...
```lua
    ...
    local function drawText(text, position)
        ...
        exports["qb-textui"]:displayTextUI(text, position)
    end

    local function changeText(text, position)
        if type(position) ~= 'string' then position = 'left' end
        exports['qb-textui']:changeText(text, position)
    end
...
```lua
    function ESX.TextUI(message, notifyType)
        if GetResourceState("qb-textui") ~= "missing" then
            return exports["qb-textui"]:displayTextUI(message, notifyType)
        end
        print("[^1ERROR^7] ^5qb-textui^7 is Missing!")
    end

    function ESX.HideUI()
        if GetResourceState("qb-textui") ~= "missing" then
            return exports["qb-textui"]:hideTextUI()
        end
        print("[^1ERROR^7] ^5qb-textui^7 is Missing!")
    end
```