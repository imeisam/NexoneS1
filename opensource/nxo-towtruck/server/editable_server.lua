...
Citizen.CreateThread(function()
    ...
    elseif Config.Framework == "qbcore" then
        ...
        for i = 1, 10 do
            print("[0Resmon Scripts] - The selected framework could not be found.")
        end
    end
end)
...
Koci.getPlayer = function(playerId)
    if Config.Framework == "esx" or Config.Framework == "oldesx" then
        ...
        self.showNotification = function(msg)
            return TriggerClientEvent("0r-towtruck:client:notification", playerId, msg)
        end
...
RegisterCommand(Config.Commands.FlatBed, function(source, args, rawCommand)
    ...
    if requests[source] and requests[source].time and requests[source].time + Config.JobOptions.callCooldown > time then
        return TriggerClientEvent("0r-towtruck:client:notification", source, Koci.getPhrase("already-called"))
    end
    ...
    TriggerClientEvent("0r-towtruck:client:callTowTruck", -1, source, GetEntityCoords(GetPlayerPed(source)))
end)
