...
local function SendNotification(playerSource, notification)
    if playerSource then
        ...
        AddOfflineNotification(notification, notification.data.phoneImei)
    end
end
...
local function sendMessage(source, channelId, company, message, anonymous, phoneImei, phoneNumber, showLocation)
    ...
    if messageResult then
        local timestampResult = MySQL.single.await(
            'SELECT `timestamp` FROM `yphone_companies_messages` WHERE `id` = ?',
            { messageId })
        local messageTimestamp = timestampResult?.timestamp
