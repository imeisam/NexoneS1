...
local function openMenu()
    ...
    local calls = lib.callback.await('qb-dispatch:callback:getCalls', false)
    if #calls == 0 then
        QBCore.Functions.Notify(locale('no_calls'), 'error')
    else
...
local function setWaypoint()
    ...
    local data = lib.callback.await('qb-dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.type) then
        ...
        TriggerServerEvent('qb-dispatch:server:attach', data.id, PlayerData)
        QBCore.Functions.Notify('Last Call Marked', 'success')
...
-- Events
RegisterNetEvent('qb-dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
...
RegisterNetEvent('qb-dispatch:client:openMenu', function(data)
    if not isJobValid(PlayerData.job.type) then return end
...
RegisterNUICallback("detachUnit", function(data, cb)
    TriggerServerEvent('qb-dispatch:server:detach', data.id, PlayerData)
    DeleteWaypoint()
    cb("ok")
end)
...
RegisterNUICallback("toggleMute", function(data, cb)
    ...
    QBCore.Functions.Notify(locale('alerts') .. muteStatus, 'info')
...
RegisterNUICallback("clearBlips", function(data, cb)
    QBCore.Functions.Notify(locale('blips_cleared'), 'success')
...
RegisterNUICallback("refreshAlerts", function(data, cb)
    QBCore.Functions.Notify(locale('alerts_refreshed'), 'success')
    local data = lib.callback.await('qb-dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
