...
RegisterNetEvent('qb-djbooth:server:playMusic', function(song, zoneName)
    ...
    if dist > Config.Locations[zoneName].radius then return end
    if QBCore.Functions.GetPlayer(src).PlayerData.job.name ~= Config.Locations[zoneName].job then return end
...
RegisterNetEvent('qb-djbooth:server:stopMusic', function(data)
    ...
    local dist = #(coords - boothCoords)
    if dist > Config.Locations[data.zoneName].radius then return end
    if QBCore.Functions.GetPlayer(src).PlayerData.job.name ~= Config.Locations[data.zoneName].job then return end
