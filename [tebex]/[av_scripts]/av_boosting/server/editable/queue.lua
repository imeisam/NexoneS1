...
function GiveContract(identifier)
    ...
    if Player then
        ...
        if not data.level then
            data.level = 0
            -- print("[ERROR] Boosting level for player "..identifier.." is null (?)")
        end
        ...
        local vinPrice = math.random(Config.Classes[currentClass]["Prices"]['vinscratch']['min'],Config.Classes[currentClass]["Prices"]['vinscratch']['max'])
        ...
        TriggerClientEvent('av_laptop:refreshContracts',tonumber(Player),contracts[identifier])
    else
