CreateThread(function()
    if Config.Item.Inventory ~= "qb-inventory" and Config.Item.Inventory ~= "lj-inventory" and Config.Item.Inventory ~= "ps-inventory" or not Config.Framework == 'qb' or not Config.Item.UniquePhones or not Config.Item.Require then return end
    ...
    RegisterNetEvent("yboomer:client:use-phone-item", function(phoneItem)
        ...
        if active then ToggleOpen(true) end
    end)

    RegisterNetEvent('yboomer:client:phone-item-added', function()
        debugPrint('Phone item added')
    end)

    RegisterNetEvent('yboomer:client:phone-item-removed', function()
        Wait(500)
