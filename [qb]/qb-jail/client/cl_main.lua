...
PrisonPoly:onPlayerInOut(function(isPointInside, point)
    ...
    if isPointInside and Config.AntiHelicopter and not (PlayerData.job.name == 'police' or PlayerData.job.name == 'justice' or PlayerData.job.name == 'ambulance') then
        triggerAntiHeli()
...
AddEventHandler('onResourceStop', function(resource)
    ...
    destroyJobBlips()
    for i=1, #workoutObjects do 
        if DoesEntityExist(workoutObjects[i]) then
            DeleteEntity(workoutObjects[i])
        end
    end
    ...
    -- for i=1, #kichenobject do 
    --     if DoesEntityExist(kichenobject[i]) then
    --         DeleteEntity(kichenobject[i])
    --     end
    -- end
    
    for i=1, #gardenobjects do 
        if DoesEntityExist(gardenobjects[i]) then
            DeleteEntity(gardenobjects[i])
        end
        exports['qb-interact']:RemoveInteraction("prison_farming_" .. i)
    end
...
RegisterNetEvent('qb-jail:client:PrisonRevive', function()
    if PlayerData.metadata.injail ~= 0 or PlayerData.job.name == 'police' then
...
CreateThread(function()
    ...
    exports['qb-target']:AddBoxZone('prison_services', vector3(1828.75, 2579.79, 46.01), 0.6, 0.5, {
        ...
        options = { 
            ...
            {
                type = 'client',
                event = 'sam-payphone:client:showpayphone',
    ...
    exports['qb-target']:SpawnPed({
        ...
        coords =vector4(1840.361328125, 2577.68359375, 45.014305114746, 359.86251831055),
        minusOne = true,
        freeze = true,
        invincible = true,
        blockevents = true,
