...
--     end)
-- end
RegisterNetEvent("phoenix:startmission:easy")
AddEventHandler("phoenix:startmission:easy", function()
    startmission('easy')
    TriggerServerEvent("phoenix:starttraining")
end)
RegisterNetEvent("phoenix:startmission:normal")
AddEventHandler("phoenix:startmission:normal", function()
    startmission('normal')
    TriggerServerEvent("phoenix:starttraining")
end)

RegisterNetEvent("phoenix:startmission:hard")
AddEventHandler("phoenix:startmission:hard", function()
    startmission('hard')
    TriggerServerEvent("phoenix:starttraining")
end)

RegisterNetEvent("phoenix:stopmission")
AddEventHandler("phoenix:stopmission", function()
    if inmission then
        endmission(true)
    end
end)
...
function startmission(diff)
    globaldiff = diff
    AddRelationshipGroup('pedtraning')
    if Config.MaxEnemies == nil then
        for k,v in pairs(Config.Enemies) do
            ...
            SetRelationshipBetweenGroups(0, GetHashKey("pedtraning"), GetHashKey("pedtraning"))
            SetRelationshipBetweenGroups(5, GetHashKey("pedtraning"), GetHashKey("PLAYER"))
            SetRelationshipBetweenGroups(5, GetHashKey("PLAYER"), GetHashKey("pedtraning"))
            ...
            if diff == 'easy' then
                SetPedCombatAbility(ped, 80) 
            elseif diff == 'normal' then 
                SetPedArmour(ped, 50)
                SetPedCombatAbility(ped, 90)
