...
RegisterNetEvent('nxte-yacht:anim:hack1', function()
    ...
    Wait(2000)
    local result = exports['nxo-MiniGames']:alphabet(10,20000)
    if result then
        ...
        OnHack1Done(result)
        TriggerServerEvent("qb-yatch:server:GetPlayerName")
        exports['qb-dispatch']:UnionRobbery(result)
    else
        NetworkStartSynchronisedScene(netScene3)
        NetworkStopSynchronisedScene(netScene3)
        DeleteObject(bag)
...
RegisterNetEvent('qb-yatch:client:LogObjectSpawn')
AddEventHandler('qb-yatch:client:LogObjectSpawn', function(playerName)
    TriggerServerEvent("qb-log:server:CreateLog", "yatch", "Heist Yatch", "Green", "**"..playerName .. "** yatch hack success")
end)
...
RegisterNetEvent('nxte-yacht:anim:hack2', function()
    ...
    local result = exports['nxo-MiniGames']:words(5,20000)
    if result then
        NetworkStartSynchronisedScene(netScene3)
        NetworkStopSynchronisedScene(netScene3)
        DeleteObject(bag)
