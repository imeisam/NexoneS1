...
izzy.debug = function(str)
    if cfg?.debug or not cfg then
        if IsDuplicityVersion() then
            print('[^1izzy^0] - [^1Debug^0]: ' .. str)
        else
            print('[izzy] - [Debug]: ' .. str)
        end
    end
end
...
if not IsDuplicityVersion() then --client
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':client:notification', izzy.notification)

    izzy.clientCallbacks = {}
    izzy.serverCallbacks = {}

    izzy.triggerCallback = function(name, cb, ...)
        izzy.serverCallbacks[name] = cb
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':client:triggerCallback', function(name, ...)
        if izzy.serverCallbacks[name] then
            izzy.serverCallbacks[name](...)
            izzy.serverCallbacks[name] = nil
        end
    end)

    izzy.createClientCallback = function(name, cb)
        izzy.clientCallbacks[name] = cb
    end
    
    izzy.triggerClientCallback = function(name, cb, ...)
        if not QBCore.ClientCallbacks[name] then return end
        QBCore.ClientCallbacks[name](cb, ...)
    end

    izzy.registerKeyMap = function(data, cb, cb2)
        RegisterCommand('+izzy_' .. data.command, function()
            local response = true
        ...
        if data.key:match('mouse') or data.key:match('iom') then
            RegisterKeyMapping('+izzy_' .. data.command, data.description, 'mouse_button', data.key:lower())
        else
            RegisterKeyMapping('+izzy_' .. data.command, data.description, 'keyboard', data.key:lower())
        end

        Wait(500)
        TriggerEvent('chat:removeSuggestion', ('/+izzy_%s'):format(data.command))
        TriggerEvent('chat:removeSuggestion', ('/-izzy_%s'):format(data.command))
    end
    ...
    izzy.getPlayersKeyPreference = function(command, cb)
        local key = GetControlInstructionalButton(0, joaat('+izzy_' .. command) | 0x80000000, true):sub(3)
        if specialChars[key] then key = specialChars[key] end
        if cb then cb(key) else return key end
    end

    izzy.drawText3D = function(x, y, z, str, length, r, g, b, a)
    ...
    izzy.addBlip = function(coords, sprite, scale, color, str, cb)
    ...
    izzy.drawSubtitle = function(str, time)
        BeginTextCommandPrint('STRING')
        AddTextComponentSubstringPlayerName(str)
        EndTextCommandPrint(time or 4000, 1)
    end

    izzy.drawBusySpinner = function(str)
        BeginTextCommandBusyspinnerOn('STRING')
        AddTextComponentSubstringPlayerName(str)
        EndTextCommandBusyspinnerOn(3)
    end

    izzy.loadAnimDict = function(dict, cb)
    ...
    izzy.loadPtfxAsset = function(asset, cb)
    ...
    izzy.loadAnimSet = function(set, cb)
    ...
    izzy.requestModel = function(model, cb)
    ...
    izzy.spawnObject = function(model, coords, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)

        izzy.requestModel(model, function()
    ...
    izzy.spawnPed = function(model, coords, heading, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)
    ...
    izzy.spawnVehicle = function(model, coords, heading, isLocal, cb)
        model = type(model) == 'number' and model or joaat(model)
    ...
    izzy.deleteObject = function(object, cb)
        SetEntityAsMissionEntity(object, false, true)
        DeleteObject(object)
        if cb then cb() else return end
    end

    izzy.deleteVehicle = function(vehicle, cb)
        SetEntityAsMissionEntity(vehicle, false, true)
        DeleteVehicle(vehicle)
        if cb then cb() else return end
    end

    izzy.deletePed = function(ped, cb)
        SetEntityAsMissionEntity(ped, false, true)
        DeletePed(ped)
        if cb then cb() else return end
    end

    izzy.getVehicles = function()
        return GetGamePool('CVehicle')
    end

    izzy.getObjects = function()
        return GetGamePool('CObject')
    end

    izzy.getPlayers = function()
        return GetActivePlayers()
    end

    izzy.getClosestPed = function(coords)
    ...
    izzy.getClosestPeds = function(coords, maxDistance)
        local ped = PlayerPedId()
        local peds = GetGamePool('CPed')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
    ...
    izzy.getClosestPlayer = function(coords)
        local ped = PlayerPedId()
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local closestPlayers = izzy.getPlayersFromCoords(coords)
    ...
    izzy.getPlayersFromCoords = function(coords, maxDistance)
        local players = izzy.getPlayers()
        local ped = PlayerPedId()
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
        local closePlayers = {}
    ...
    izzy.getClosestVehicle = function(coords)
        local ped = PlayerPedId()
    ...
    izzy.getClosestVehicles = function(coords, maxDistance)
        local ped = PlayerPedId()
        local vehicles = GetGamePool('CVehicle')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
    ...
    izzy.getClosestObject = function(coords)
        local ped = PlayerPedId()
    ...
    izzy.getClosestObjects = function(coords, maxDistance)
        local ped = PlayerPedId()
        local objects = GetGamePool('CObject')
        coords = coords and (type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords) or GetEntityCoords(ped)
        local maxDistance = maxDistance or 5
    ...
    local duis = {}
    izzy.createDui = function(url, width, height, cb)
        width = width or 512
    ...
    izzy.changeDuiUrl = function(id, url)
        if not duis[id] then
    ...
    izzy.raycast = function(origin, target, options, ignoreEntity, radius)
    ...
    izzy.getEntityPlayerIsLookingAt = function(maxDistance, radius, flag, ignore)
        ...
        local _, hit, targetCoords, _, targetEntity = izzy.raycast(originCoords, forwardCoords, flag or 4294967295, ignore, radius or 0.2)
    ...
    izzy.getEntityInFrontOfEntity = function(entity, maxDistance, radius, flag)
        local maxDistance = maxDistance or 3.0
        ...
        local _, hit, _, _, targetEntity = izzy.raycast(originCoords, targetCoords, flag or 4294967295, entity, radius or 0.2)
        return targetEntity
    end

    izzy.getVehicleProperties = function(vehicle)
        if DoesEntityExist(vehicle) then
    ...
    izzy.setVehicleProperties = function(vehicle, props)
        if DoesEntityExist(vehicle) then
    ...
    izzy.registerCallback = function(name, cb)
        izzy.serverCallbacks[name] = cb
    end

    izzy.triggerCallback = function(name, src, cb, ...)
        if izzy.serverCallbacks[name] then
            izzy.serverCallbacks[name](src, cb, ...)
        else
            izzy.debug('This callback(^2' .. name .. '^0) is not registered!')
        end
    end
    ...
    RegisterNetEvent(GetCurrentResourceName() .. ':server:triggerCallback', function(name, ...)
        ...
        izzy.triggerCallback(name, src, function(...)
    ...
    izzy.notification = function(src, type, str, length)
        TriggerClientEvent(GetCurrentResourceName() .. ':client:notification', src, type, str, length)
    end

    izzy.getIdentifiers = function(src, identifiertypes)
    ...
    izzy.sendLog = function(webhookURL, color, str, imgUrl)
        if webhookURL and webhookURL ~= '' then
            ...
            local data = {
                ...
                ['embeds'] = { {
                    ['title'] = 'izzy-store.tebex.io',
                    ['url'] = 'https://izzy-store.tebex.io/',
                    ['color'] = logColors[color] and logColors[color] or logColors['default'],
                    ['timestamp'] = os.date('!%Y-%m-%dT%H:%M:%SZ'),
                } },
                ['footer'] = {
                    ['text'] = 'izzy-store.tebex.io',
            ...
            izzy.debug('Webhook URL is empty!')
        end
    end

    izzy.sendSelfLog = function(src, webhookURL, color, str, imgUrl)
        if webhookURL and webhookURL ~= '' then
            if src then
                local name = sanitize(GetPlayerName(src))
                local identifiers = izzy.getIdentifiers(src, { 'license', 'discord' })
                local text = ''
