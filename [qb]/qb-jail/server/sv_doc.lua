QBCore.Commands.Add('reducesentence', 'Reduce Player Sentence', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Time in minutes'}}, true, function(source, args)
    ...
    if Player and OtherPlayer and amount and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then -- if Player and OtherPlayer and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then
