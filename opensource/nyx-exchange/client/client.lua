...
LaptopAnimation = function(data) -- Globally used
    local loc = vector3(427.04, -1900.9, 27.61)
    ...
    if item then
        ...
        Wait(2000)
        local memorygame = exports['nxo-MiniGames']:words(5, 15000)
        local doorData = Config.PowerLocations['doorhack']
        if memorygame then
            ...
            TriggerServerEvent('nyx-exchange:server:setbusystate', 'doorhack', true)
...
CreateTrollys = function()
    ...
    for k, v in pairs(Config.trollys.trolly) do
...
LootTrolly = function(index) -- Globally used
    -- set taken
    TriggerServerEvent('nyx-exchange:server:SetTrollyBusy', index, true)
    -- animation
    LocalPlayer.state:set("inv_busy", true, true)

    -- Determine the type of trolly (money or gold)
    local trollyType = Config.trollys.trolly[index].type
    local coords = Config.trollys.trolly[index].coords
    local ped = PlayerPedId()
...
RegisterNetEvent('nyx-exchange:SmartLock', function(id, data)
    local playerCoords = vector4(438.43, -1904.76, 25.93, 32.98)
    local usbCoords = vector3(437.912262, -1904.46094, 26.2491665)
    local hasCops = lib.callback.await('nyx-exchange:hasenoughcops', false)
    local alarm_coords = vector3(433.3, -1896.59, 28.42)
    local item = QBCore.Functions.HasItem(data.requireditem, 1)
    if hasCops then
        if item then
            StartPhoneAnimation(playerCoords, usbCoords)
            TriggerServerEvent('nyx-exchange:server:setbusystate', id, true)
            QBCore.Functions.Progressbar("avalin_generator", "Disabling Smart Lock...", 4500, false, true, {
                ...
                exports["nxo-oui"]:Show('Strawberry Cash Exchange',"Disable The Remote Door Locks")
                TriggerServerEvent('nyx-exchange:togglealarm', 'alarm', true, alarm_coords)
                TriggerAlert()
            end, function()
                FinishPhoneAnimation()
        ...
        TriggerEvent("QBCore:Notify", "Not Enough Cops Right Now", "error")
    end
end)


RegisterNetEvent('nyx-exchange:DoorLock', function(id, data)
    local playerCoords = vector4(432.58, -1906.81, 25.98, 136.56)
    local usbCoords = vector3(432.305969, -1907.07056, 26.3831558)
    local item = QBCore.Functions.HasItem(data.requireditem, 1)
    if item then
        StartPhoneAnimation(playerCoords, usbCoords)
        local alphabet = exports['nxo-MiniGames']:alphabet(20, 20000)
        if alphabet then
            TriggerServerEvent('nyx-laundromat:togglealarm', 'alarm', false)
            TriggerServerEvent('nyx-exchange:server:setbusystate', id, true)
            for key, doorId in pairs(data.doorids) do
                TriggerServerEvent('qb-doorlock:server:updateState', doorId, false, false, false, true, false, false)
            end
...
RegisterNetEvent('nyx-exchange:client:setbusystate', function(id, state)
...
CreateThread(function()
        ...
        options = {
            {
                ...
                action = function(entity, coords, args)
                    TriggerEvent("nyx-exchange:SmartLock", 'generator', generatorData)
                end,
    ...
    local powerData = Config.PowerLocations['alarm']
    exports['qb-interact']:AddInteraction({
...
RegisterNetEvent('nyx-exchange:client:SetTrollyBusy', function(index, state)
    Config.trollys.trolly[index].taken = state
end)


RegisterNetEvent('nyx-exchange:reset', function(id, data)
    ...
    TriggerServerEvent('nyx-exchange:togglealarm', 'power', false)
    TriggerServerEvent('nyx-exchange:server:setbusystate', 'generator', false)
    TriggerServerEvent('nyx-exchange:server:setbusystate', 'alarm', false)
    TriggerServerEvent('nyx-exchange:server:setbusystate', 'doorhack', false)
end)
...
CreateThread(function()
    for i, trolly in pairs(Config.trollys.trolly) do
        exports['qb-interact']:AddInteraction({
            coords = trolly.coords.xyz,
            distance = 1.5,
            interactDst = 1.0,
            id = 'trolly_' .. i,
            options = {
                {
                    label = 'Grab ' .. trolly.type,
                    action = function(entity, coords, args)
                        LootTrolly(i)
                    end,
                    canInteract = function(entity, coords, args)
                        return not trolly.taken
                    end,
                },
            }
        })
