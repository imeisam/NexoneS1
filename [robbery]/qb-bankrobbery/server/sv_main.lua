...
RegisterNetEvent('qb-bankrobbery:server:SetBankHacked', function(bank)
    ...
    TriggerEvent("qb-cooldown:server:startglobaltimeout", 45)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if debug then
...
RegisterNetEvent('qb-bankrobbery:server:TrollyReward', function(type)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local info = {worth = 0} -- money bag worth
    ...
    elseif type == 'fleeca' then
        local receiveAmount = math.random(25000, 40000)
    ...
    elseif type == 'paleto' then
        local receiveAmount = math.random(40, 60)
        Player.Functions.AddItem('markedbills', receiveAmount, false, info)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add', receiveAmount)
        TriggerClientEvent('QBCore:Notify', src, 'You got ' .. receiveAmount .. ' bags of inked money...')
    ...
    elseif type == 'pacific' then
        local receiveAmount = math.random(200, 400)
        Player.Functions.AddItem('markedbills', receiveAmount, false, info)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add', receiveAmount)
        TriggerClientEvent('QBCore:Notify', src, 'You got ' .. receiveAmount .. ' bags of inked money...')
...
RegisterNetEvent('qb-bankrobbery:server:LockerReward', function(type)
    ...
    if type == 'fleeca' then
        ...
        local tier = 1
        if tierChance <= 20 then tier = 1 elseif tierChance > 20 and tierChance <= 40 then tier = 2 elseif tierChance > 80 and tierChance <= 95 then tier = 3 else tier = 4 end
        ...
        if tier == 1 then
            ...
            local info = {
                ...
                Player.Functions.AddItem("blueprint", itemAmount, false, info)
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["blueprint"], "add")

            TriggerEvent('qb-log:server:CreateLog', 'bankrobbery', 'Bankrobbery log: '..type, 'green', '**'.. Player.PlayerData.name .. '** (citizenid: *'..Player.PlayerData.citizenid..'* | id: *'..src..'*) received: ** Blueprint '..itemAmount.. 'x '..info..'**')
    ...
    elseif type == 'paleto' then
        ...
        elseif tier == 3 then
            --grey usb
            Player.Functions.AddItem('usb_grey', 1)
            TriggerEvent('qb-log:server:CreateLog', 'bankrobbery', 'Bankrobbery log: '..type, 'blue', '**'.. Player.PlayerData.name .. '** (citizenid: *'..Player.PlayerData.citizenid..'* | id: *'..src..'*) received: **usb_grey**')
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['usb_grey'], 'add')
            if debug then
                print('^3[qb-bankrobbery] ^5'..Player.PlayerData.name..'(citizenid: '..Player.PlayerData.citizenid..' | id: '..src..') received usb_grey^7') 
            end
        elseif tier == 4 then
            --red usb
            local chance = math.random(1,2)
            if chance == 2 then
                Player.Functions.AddItem('laptop_red', 1)
                TriggerEvent('qb-log:server:CreateLog', 'bankrobbery', 'Bankrobbery log: '..type, 'green', '**'.. Player.PlayerData.name .. '** (citizenid: *'..Player.PlayerData.citizenid..'* | id: *'..src..'*) received: **laptop_red**')
                TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['laptop_red'], 'add')
                if debug then
                    print('^3[qb-bankrobbery] ^5'..Player.PlayerData.name..'(citizenid: '..Player.PlayerData.citizenid..' | id: '..src..') received laptop_red^7') 
                end
            else
                Player.Functions.AddItem('security_card_02', 1)
                TriggerEvent('qb-log:server:CreateLog', 'bankrobbery', 'Bankrobbery log: '..type, 'green', '**'.. Player.PlayerData.name .. '** (citizenid: *'..Player.PlayerData.citizenid..'* | id: *'..src..'*) received: **security_card_02**')
...
QBCore.Functions.CreateCallback('qb-bankrobbery:server:RobberyBusy', function(source, cb, bank)
    if disable then
        ...
        if not exports['qb-cooldown']:isCooldown() or not robberyBusy[bank] then
            cb(false)
        else
            cb(true)
        end
...
QBCore.Functions.CreateUseableItem("laptopg", function(source, item)
...
CreateThread(function() -- Doorlock check
    if Config.DoorLock == 'qb' or Config.DoorLock == 'nui' then return end
