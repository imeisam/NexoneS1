...
RegisterNetEvent('nyx-exchange:processTrade', function(trade, quantity)
    local src = source
    ...
    if Player then
        ...
        if item and item.amount >= quantity then
            ...
            TriggerClientEvent('QBCore:Notify', src, 'Trade successful! You received ' .. rewardAmount .. ' ' .. trade.reward, 'success')
        else
