local officerdowncooldwon = false
local timer = {}
...
AddEventHandler('CEventGunShot', function(witnesses, ped)
    if exports["nxo-paintball"]:IsInPaintball() then return end
    ...
    WaitTimer('Shooting', function()
        ...
        if witnesses and not isPedAWitness(witnesses, ped) then return end
            if cache.vehicle then
                exports['qb-dispatch']:VehicleShooting()
            else
...
for i = 1, #SpeedingEvents do
    ...
    AddEventHandler(event, function(_, ped)
        WaitTimer('Speeding', function()
            ...
            exports['qb-dispatch']:SpeedingVehicle()
            SpeedTrigger = GetGameTimer()
        end)
    end)
end

RegisterNetEvent('qb-dispatch:client:panic', function()
    if not officerdowncooldwon then
        if QBCore.Functions.HasItem('pdradio', 1, true) then 
            TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, 'panicbutton', 0.4)
            exports['qb-dispatch']:OfficerBackup()
            officerdowncooldwon = true
            SetTimeout(7000, function()
                officerdowncooldwon = false
            end)
        else
            QBCore.Functions.Notify('You don not have Panic Buttom', 'error')
        end
    else
        QBCore.Functions.Notify('You can not spam Panic Buttom', 'error')
    end
end)
...
RegisterNetEvent('qb-dispatch:client:officerdown', function()
    if not officerdowncooldwon then
        if QBCore.Functions.HasItem('pdradio', 1, true) then
            TriggerServerEvent("InteractSound_SV:PlayWithinDistance", 10, 'panicbutton', 0.4)
            exports['qb-dispatch']:OfficerInDistress()
            officerdowncooldwon = true
            SetTimeout(7000, function()
                officerdowncooldwon = false
            end)
...
RegisterNetEvent('qb-dispatch:client:officerdown2', function()
    if not officerdowncooldwon then
        if QBCore.Functions.HasItem('pdradio', 1, true) then 
            exports['qb-dispatch']:OfficerDown()
            officerdowncooldwon = true
