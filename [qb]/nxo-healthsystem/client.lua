...
function gethealthbone()
    return current.status
end

exports('gethealthbone',gethealthbone)
...
RegisterCommand("opent", function()
    TriggerEvent("lvs_clothing:client:OpenClothingShop")
end)

local injoritem={"bullet","knife","unarmed"}
local isblood=false
CreateThread(function()
    while true do 
        if isloaded then
            TriggerEvent("izzy-healthsystem:client:setmetadata")
        end
        Citizen.Wait(10000) 
    end
end)
...
function handleDisplay()
    ...
    if current.opened then
        ...
        local playerId, distance = izzy.getClosestPlayer()
        if not playerId then playerId = PlayerId() else TriggerServerEvent("izzy-healthsystem:server:sendPlayerData", GetPlayerServerId(playerId)) end
        Wait(500)
        initialize(playerId)
    end
end
...
function updateInventoryItems()
    izzy.triggerCallback("izzy-healthsystem:getInventoryItems", function(res)
...
function initialize(playerId)
    ...
    if  current.isDead  then
        ...
        SetCamFov(current.cam, current.isDead and 12.0 or 15.0)
    end
...
function close()
    ...
    if current.ped ~= PlayerPedId() then
        TriggerServerEvent("izzy-healthsystem:server:savePlayer", GetPlayerServerId(current.playerId), current)
...
RegisterCommand("openht", function()
    ...
    if (cfg.accessableJobs[job] or cfg.accessableJobs["all"]) then
        ...
        izzy.notification("error", izzy._t("youCantAccess"))
    end
end)

RegisterNetEvent("izzy-healthsystem:client:setPlayerData", function(playerData)
    current.oldData = {
...
RegisterNetEvent("izzy-healthsystem:client:savePlayer", function(playerData)
    current.status = playerData.status
    current.hitSide = playerData.hitSide
    current.isDead = playerData.isDead
end)

RegisterNetEvent("izzy-healthsystem:client:getPlayerData", function(doctor)
    TriggerServerEvent("izzy-healthsystem:server:setPlayerData", doctor, current)
end)

RegisterNetEvent("izzy-healthsystem:client:setmetadata", function()
    TriggerServerEvent("izzy-healthsystem:server:setmetadata", current)
end)
...
AddEventHandler("gameEventTriggered", function(event, data)
    if event == "CEventNetworkEntityDamage" then
        ...
        if PlayerPedId() == victim then
            ...
            local damage1 = 0   
            local hitItem = izzy.weapons[wp] or "knife"
            ...
            if isMelee then
                damage1= cfg.damages[wp] and cfg.damages[wp]() or math.random(5, 40)
            else
...
RegisterNUICallback("healBone", function(data, cb)
    ...
    if current.status[bone] >= 100 then return izzy.notification("error", izzy._t("healthyBone")) end
    
    if string.find(bone, "Arm") then
        izzy.loadAnimDict("mp_gun_shop_tut")
        TaskPlayAnim(ped, "mp_gun_shop_tut", "indicate_right", 2.0, 2.0, -1, true, 0, false, false, false)
    elseif string.find(bone, "Leg") then
        izzy.loadAnimDict("gestures@f@standing@casual")
    ...
    if canRespawn() then
        TriggerServerEvent("izzy-healthsystem:server:revivePlayer", GetPlayerServerId(current.playerId))
    ...
    TriggerServerEvent("izzy-healthsystem:server:removeItem", item, 1)
    izzy.notification("success", izzy._t("successfulAction"))
    updateInventoryItems()
    Wait(500)
...
RegisterNetEvent("izzy-healthsystem:client:ifak", function()
    for bone, _ in pairs(current.status) do
        if current.status[bone]+25>100 then
            current.status[bone] =100
        else
            current.status[bone] = current.status[bone]+ 25
        end
    end
end)

RegisterNetEvent("izzy-healthsystem:client:medkit", function()
    
    for bone, _ in pairs(current.status) do
        if current.status[bone]+15>100 then
            current.status[bone]=50
        else
            current.status[bone] = current.status[bone]+ 15
        end
    end
end)

RegisterNetEvent("izzy-healthsystem:client:revive", function()
    current.isDead = false
    current.hitSide = {}
    for bone, _ in pairs(current.status) do
        current.status[bone] = 100
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    current=QBCore.Functions.GetPlayerData().metadata['healthsystem']
    if not current then
        current = {
            opened = false,
            cam = nil,
    ...
    -- current.ped=PlayerPedId() 
    -- current.playerId=GetPlayerServerId(PlayerId())
    isloaded=true
end)