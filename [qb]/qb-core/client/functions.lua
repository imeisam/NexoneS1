...
function QBCore.Functions.GetCash()
    local count = 0
    for i=1, QBConfig.Player.MaxInvSlots do
        local data = QBCore.PlayerData.items[i]
...
function QBCore.Functions.HasItem(item, amount, withbackpack, noQuality)
...
function QBCore.Functions.HasDivision(division)
    if not division or not QBCore.PlayerData.job or not QBCore.PlayerData.job.divs then return false end
	return QBCore.PlayerData.job.divs[division]
end
...
local subtype
RegisterNetEvent('qb-core:server:getsub', function(subname)
    subtype = subname
end)

function QBCore.Functions.Getsub()
    return subtype
end