...
window.addEventListener('message', function(event) {
	...
	if (ed.action === "openMenu") {
		...
		document.getElementById("invoice_amount_input").innerHTML=translations.invoice_amount_input;
...
function clFunc(name1, name2, name3, name4, name5) {
    if (name1 === "showAllBillInfos") {
				...
				if (existingBill.paid) {
					...
					<div id="mainDivInsideBillDivInside" style="align-items: flex-start;">
						<div id="MDIRDIDiv1" style="padding-top: 0;">
							<div class="MDIRDIDivTextDiv"><span>${amountText}</span><span>$${existingBill.amount}</span></div>
						</div>
		...
		xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
		xhr.setRequestHeader('Content-Type', 'application/json');
		xhr.send(JSON.stringify({action: "sendInvoice", target: invoicePlayer, title: title.value, price: Number(price.value)}));
		invoiceMenu = false;
		$("#billingInputDiv").show().css({bottom: "0%", top: "0%"}).animate({bottom: "-200%"}, 800, function() {});
		var xhr = new XMLHttpRequest();
		xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
		...
		xhr.open("POST", `https://${GetParentResourceName()}/callback`, true);
		xhr.setRequestHeader('Content-Type', 'application/json');
		xhr.send(JSON.stringify({action: "payBill", id: Number(name2), amount: Number(name3), type: name4, sender: name5}));
	}
}
