...
local function SpawnVehicle(carType, location, group, coords, src)
    local src = source
...
RegisterNetEvent('Renewed-Garbage:server:StartJob', function(run, coords)
    ...
    local notify = TriggerClientEvent('QBCore:Notify', src, "Excellent, the garbage vehicle is waiting for you.", "success")
...
RegisterNetEvent('Renewed-Garbage:server:MaterialTrade', function(material)
    ...
    if Player.Functions.AddItem(material, math.random(Config.Trade[material].min ,Config.Trade[material].max ) ) then
