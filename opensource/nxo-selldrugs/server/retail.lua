RegisterNetEvent("SellDrugs:Status_Selling_Drugs", function(src)
    ...
    if not src and not source then
        print("This trigger must be used from the client side or you need to specify the player as an argument. If you're unable to do this, please contact Support - discord.gg/URKNMSwx5W")
        return 
    end
...
RegisterNetEvent("SellDrugs:Make_Trade", function(Drug, Amount)
    ...
    if #Player["id"..src].Prices == 0 or not item then
        ...
        Send_Discord_Log(3447003, translate("server", "discord_hacker_title", GetPlayerName(src), src, xPlayer.PlayerData.citizenid), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_hacker_make_trade_message", full_name), "Support - discord.gg/URKNMSwx5W")
        return
    end
    ...
    if item and item.amount >= Amount then
        ...
        Player["id"..src].Prices = {}
        local full_name = xPlayer.PlayerData.charinfo.firstname.." "..xPlayer.PlayerData.charinfo.lastname
        Send_Discord_Log(3447003, translate("server", "discord_retail_sell_title", GetPlayerName(src), src, xPlayer.PlayerData.citizenid), "inside-scripts (Advanced Drug Sales 💊)", translate("server", "discord_retail_sell_message", full_name, reformatInt(Amount), item.label, reformatInt(Price)), "Support - discord.gg/URKNMSwx5W")
    end
end)
