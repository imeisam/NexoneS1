...
local RCCar = {}
local player = nil
local LoseConnectionDistance = 100.0
local damaga = 100
local Attached = false

RegisterNetEvent('skateboard:Spawn')
AddEventHandler('skateboard:Spawn', function(qul)
	if qul then
		damaga = qul
	end
	local ped = PlayerPedId
	QBCore.Functions.Progressbar('place_board', 'Placing Board', 1000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function()
		RCCar.Start() -- Play When Done
		--Stuff goes here
	end, function()
		local ped = PlayerPedId
		StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0) -- Play When Cancel
		--Stuff goes here
	end)
    RCCar.Start()
end)
...
RCCar.HandleKeys = function(distanceCheck)
	if distanceCheck <= 5.5 then
		if IsControlJustPressed(0, 38) then
			-- exports['qb-core']:KeyPressed()
			QBCore.Functions.Progressbar('pick_board', 'Picking Up Skateboard', 1000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
				...
				RCCar.Attach("pick")
				TriggerServerEvent('skateboard:pick', damaga) 
			end, function()
	...
	if distanceCheck < LoseConnectionDistance then
		local overSpeed = (GetEntitySpeed(RCCar.Entity)*3.6) > 90
		...
		if IsControlPressed(0, 22) and Attached then
			...
			if not IsEntityInAir(RCCar.Entity) then	
				...
				local maxJumpHeigh = 6.0
				boost = maxJumpHeigh * duration / 250.0
				if boost > maxJumpHeigh then boost = maxJumpHeigh end
...
RCCar.AttachPlayer = function(toggle)
	if toggle then
		...
		Attached = true		
		-- print('ok')
		-- exports['qb-core']:DrawText('[G] Hop Off [E] Pick Up ')
