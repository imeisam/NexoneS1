            ...
            if LocalPlayer.state.isLoggedIn and not keyPressed and GetLastInputMethod(0) then
                ...
                if next(keyMeta) ~= nil then
                    if keyMeta[v[2]]["command"] ~= "" then
                        ...
                        if args[1] ~= nil then
                            ExecuteCommand(keyMeta[v[2]]["command"]..' '..args[1])
                        else
                            ExecuteCommand(keyMeta[v[2]]["command"])
                        end
                        keyPressed = true
                        Wait(1000)
                        keyPressed = false
                    else
                        QBCore.Functions.Notify('There is still nothing ['..v[2]..'] bound, /binds to bind a command', 'error', 4000)
                    end
                else
                    QBCore.Functions.Notify('You have not bound any commands, /binds to bind a command', 'error', 4000)
                end
            end
        end
    end)
