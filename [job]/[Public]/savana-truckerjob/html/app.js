...
$(document).on("click", ".closeIcon", function() { 
  $("html,body").hide()
  $.post("https://savana-truckerjob/nuiOff",function () {})
})
...
$(document).on("click", "#confirm", function () {
  if (currentJobIndex !== null) {
    ...
    $("html,body").hide();
    $.post("https://savana-truckerjob/nuiOff", function () {});
    $.post("https://savana-truckerjob/startJob", JSON.stringify(selectedJob), function () {
      console.log("Job started:", selectedJob);
    });
  } else {
...
$(document).on("click", ".trailerItem", function () {
  ...
  if ($(this).hasClass("active")) {
    ...
    const popup2 = document.querySelector(".popUpText");
    popup2.innerHTML = `
        <p>Do you agree to take the ${trailerName} job?</p>
    `;
    ...
    if (parseInt(job.level) <= parseInt(playerLevel)) {
      itemDiv.innerHTML = `
        ...
        <div id="trailerInfo">
          ...
          <div class="trailerType"><p class="jobType">${job.type}</p></div>
          <div class="trailerRoute">${job.streetNames}</div>
          <div class="trailerPrice">Total Price: $${job.totalPrice}</div>
          <div class="trailerKmPrice">Per KM Price: $${job.kmEarnings}</div>
        </div>
      `;
    } else {
      itemDiv.innerHTML = `
