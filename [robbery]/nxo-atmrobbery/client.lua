...
local function GetSuccessPoint(veh)
	...
	if ( RPM >= 0.10) and Gear >= 1 and not HandBreake and Speed >= 2.0 then
		return true
	end
	return false
end
...
local function StartedBreakATM(veh)
	if GetSuccessPoint(veh) then
		...
		if Round(Config.MaxTry/2)==Point then
			if Config.PoliceAlert then
				exports['qb-dispatch']:atmrobery()
			end
		end
		Wait(3000)
	else
...
RegisterNetEvent('qb-atmrobbery:client:UseTowingRope', function()
	...
	local ped = PlayerPedId()
	if exports["qb-smallresources"]:getdevelopermode() or CurrentCops >= Config.MinimumATMRobberyPolice then
			...
			if CheckATM then
				...
				if VehDist <= 5.0 then
					if not BlackListCar(veh) then
						TriggerServerEvent('qb-atmrobbery:cooldown')	
													...
													options = {
														{
															...
															canInteract = function()
																...
																if ObjDis <= 3.0 and not PickUpATM and not SearchATM then
																	return true
																end
																return false
															end,
														},
														{
															...
															canInteract = function()
																local ObjDis = #(GetEntityCoords(ped) - GetEntityCoords(ATMobject, false))
																if ObjDis <= 3.0 and not SearchATM and not PickUpATM then
																	return true
																end
																return false
															end,
...
RegisterNetEvent('qb-atmrobbery:SearchATMForCash', function()
	...
	QBCore.Functions.Progressbar("Search_atmCash", "Searching ATM...", Config.SearchATMTimer, false, true, {
		...
		StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
		SearchATM = true
		TriggerServerEvent('qb-atmrobbery:server:AddRewardForATMObj', 'search')
	end, function() -- Cancel
		SearchATM = false
		StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
	end)
...
RegisterNetEvent('qb-atmrobbery:client:UseATMObject', function()
	...
	if #(pos - TargetCoord) < 5.0 then
        ...
        local LockPick=exports['nxo-MiniGames']:untangle(10,30000)
		...
		if LockPick then
			QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
				if result then
					...
					TriggerServerEvent("qb-atmrobbery:server:AddRewardForATMObj",'pickup')
					GetCheckItem()
					Wait(1000)
					startrob=false
				end
...
if not startrob then return end
	GetCheckItem()
end)
