...
function CreateTradePed(pedConfig)
    ...
    exports['qb-interact']:AddInteraction({
        coords = vector3(-359.99, -1542.57, 27.74),
        distance = 5.5,
        interactDst = 1.0,
        options = {
            {
                ...
                action = function(entity, coords, args)
                    TriggerEvent("qb-trading:openMenu")
                end,
...
function OpenTradeMenu()
    ...
    for _, trade in ipairs(Config.TradeItems) do
        table.insert(tradeOptions, {
            header = "Trade " .. "Recyle Material",
            txt = "Get " .. trade.reward,
            params = {
                event = "qb-trading:openTradeInput",
                args = trade
            }
        })
    end
...
RegisterNetEvent('qb-trading:openTradeInput', function(trade)
    ...
    if dialog then
        ...
        if quantity and quantity > 0 then
            TriggerServerEvent('qb-trading:processTrade', trade, quantity)
        end
    end
end)

RegisterNetEvent('qb-trading:openMenu', function()
    OpenTradeMenu()
end)
