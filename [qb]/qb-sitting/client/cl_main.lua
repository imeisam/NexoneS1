...
local function requestAnimDict(model)
...
local function StartSittingThread()
    sitting = true
    exports['qb-core']:DrawText('Cancel','left','Q')
...
local function MakePedSitDown(coords, heading, animData)
    ...
    exports['qb-core']:DrawText('Press [Right Click] to cancel')
...
local function PlacingThread(animData)
    if ped == nil then
        ...
        if not animToUse then
            QBCore.Functions.Notify('No animation found.', 'error')
            return
        end
